...
| 39 | AgentDesk Reasonix Basic Provider | **Contract Current — TC-13.28a.2 Phase A** / Permission Policy Evidence Current (`acceptEdits` + `Bash,Read,Write,Edit`) / Runtime Wired / Decoder Fixture Verified | TC-13.28a.2 | Frozen provider identity (`"reasonix"`), API model (`"deepseek-v4-flash"`), Reasonix route (`"deepseek-flash"`), profile (`economy`), max 12 tool rounds; one-shot `reasonix run` via stdin with `--output-format json`; `--permission-mode acceptEdits --allowed-tools Bash,Read,Write,Edit` frozen by loopback-stub evidence; ACP, TUI, MCP, web, subagent, Yolo, auto, bypassPermissions, manual forbidden; API key managed exclusively by Reasonix user directory; Reasonix JSON wrapper passes its provider-specific fail-closed decoder before entering the shared `WorkerOutput` / `DeliveryReceipt` chain; full real API probe remains environment-gated; Interface #22 unchanged |
...
runtime is **Current — TC-13.26b**; E2E verification is **Verified — TC-13.26c**.
