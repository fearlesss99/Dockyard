...
5. Copy the selector's exact ten-field object to ledger `current_dispatch.model_selection` and outbox `model_selection`; no missing, extra, or renamed keys, and array values retain the selector's order.
