...
7. Allocate the attempt and IDs, run `select_model.py` with the full `task_card_commit`, and, when required, create a matching `MODEL_DEGRADATION_APPROVED` event. Copy the selector's ten fields exactly into ledger and outbox.
