"""TC-13.19f — Expert User-Decision Cancellation Closed-Loop E2E.

Deterministic E2E that validates the complete escalation chain through
all four WorkerKind tiers followed by caller-initiated cancellation after
...
Also validates that an illegal BlockedCancellationRequest is rejected
fail-closed with zero writes before any state mutation.
...
class WorkflowExpertCancellationE2ETests(unittest.IsolatedAsyncioTestCase):
    """TC-13.19f: Expert user-decision cancellation closed-loop E2E.

    One test validates: Basic → Standard → Advanced → Expert all blocked,
    Expert returns REQUEST_USER_DECISION, then caller-initiated cancellation
    via BLOCKER_CANCELLED → cancelled.  Also validates illegal cancellation
    requests fail-closed with zero writes.
    """

    async def test_expert_user_decision_cancels_blocked_task_fail_closed(
        self,
    ) -> None:
        ...
        try:
            ...
            try:
                ...
                # ═════════════════════════════════════════════════════════
                # Phase E: Expert pre-cancellation state verification
                # ═════════════════════════════════════════════════════════
                ...
                # ═════════════════════════════════════════════════════════
                # Phase F: Illegal cancellation request — fail-closed,
                #          zero writes
                # ═════════════════════════════════════════════════════════
                ...
                # Illegal: event type is not BLOCKER_CANCELLED
                # (TASK_CANCELLED + CancelledPayload is a different pathway)
                illegal_cancel_cas = TransitionCAS(
                    task_id=task_id,
                    expected_revision=revision,
                    expected_state="blocked",
                    expected_snapshot_commit=head_sha,
                )
                illegal_cancel_tr = TransitionRequest(
                    cas=illegal_cancel_cas,
                    dispatch_cas=None,
                    event_id="EVT-CANCEL-ILLEGAL-1",
                    event_type="TASK_CANCELLED",  # ← wrong event type
                ...
                illegal_req1 = BlockedCancellationRequest(
                    blocked_audit_request=blocked_audit_req_a4,
                    blocked_audit_result=blocked_audit_result_a4,
                    cancellation_transition_request=illegal_cancel_tr,
                )

                with self.assertRaises(WorkflowInputError):
                    await orch.record_blocked_cancellation(illegal_req1)

                # Zero writes
                self.assertEqual(
                    tasks_yaml_path.read_bytes(), tasks_yaml_before,
                    "tasks.yaml must be unchanged after illegal cancellation request",
                )
                self.assertEqual(
                    sorted(p.name for p in events_dir.iterdir() if p.is_file()),
                    events_before,
                    "events must be unchanged after illegal cancellation request",
                )
                self.assertEqual(
                ...
                # Second illegal: expected_state is not "blocked"
                illegal_cancel_cas2 = TransitionCAS(
                    task_id=task_id,
                    expected_revision=revision,
                    expected_state="accepted",  # ← illegal: must be "blocked"
                    expected_snapshot_commit=head_sha,
                )
                illegal_cancel_tr2 = TransitionRequest(
                    cas=illegal_cancel_cas2,
                    dispatch_cas=None,
                    event_id="EVT-CANCEL-ILLEGAL-2",
                    event_type="BLOCKER_CANCELLED",
                    payload=BlockerCancelledPayload(),
                    event_context=TransitionEventContext(
                        source_message_id=None,
                ...
                illegal_req2 = BlockedCancellationRequest(
                    blocked_audit_request=blocked_audit_req_a4,
                    blocked_audit_result=blocked_audit_result_a4,
                    cancellation_transition_request=illegal_cancel_tr2,
                )

                with self.assertRaises(WorkflowInputError):
                    await orch.record_blocked_cancellation(illegal_req2)
                ...
                # ═════════════════════════════════════════════════════════
                # Phase G: Legal cancellation request
                # ═════════════════════════════════════════════════════════

                cancel_event_id = "EVT-CANCEL-EXPERT-E2E"
                cancel_tr = _make_cancellation_transition_request(
                    task_id=task_id,
                    revision=revision,
                    head_sha=head_sha,
                ...
                cancel_req = BlockedCancellationRequest(
                    blocked_audit_request=blocked_audit_req_a4,
                    blocked_audit_result=blocked_audit_result_a4,
                    cancellation_transition_request=cancel_tr,
                )

                cancel_result = await orch.record_blocked_cancellation(cancel_req)

                # Result assertions
                self.assertIsInstance(cancel_result, BlockedCancellationResult)
                self.assertEqual(cancel_result.task_id, task_id)
                self.assertIs(
                    cancel_result.escalation_decision,
                    blocked_audit_result_a4.escalation_decision,
                    "escalation decision identity must be preserved",
                )
                self.assertEqual(
                    cancel_result.cancellation_transition.from_state, "blocked",
                )
                self.assertEqual(
                    cancel_result.cancellation_transition.to_state, "cancelled",
                )

                # No additional subprocess calls
                self.assertEqual(
                    len(subprocess_calls), 8,
                    "Subprocess calls must remain exactly 8 after "
                    "legal cancellation",
                )
                ...
                self.assertEqual(count_event("BLOCKER_CANCELLED"), 1,
                                 "Must have exactly 1 BLOCKER_CANCELLED event")
                ...
                self.assertEqual(
                    len(task_events_sorted), 20,
                    f"Expected exactly 20 events, got {len(task_events_sorted)}: {observed_types}",
                )
                ...
                expected_sequence = [
                    ...
                    "TASK_BLOCKED",          # A4
                    "BLOCKER_CANCELLED",     # Terminal cancellation
                ]

                self.assertEqual(
                    observed_types, expected_sequence,
                    "Event sequence must match exact expected order",
                )
                ...
                for ev in task_events_sorted:
                    ...
                    if ev.event_type in gated_events:
                        ...
                        "TASK_BLOCKED", "BLOCKER_RESOLVED", "BLOCKER_CANCELLED",
                    }:
                        self.assertFalse(has_approval_gate,
                                         f"{ev.event_type} must NOT have approval_gate")

                # BLOCKER_CANCELLED must not have approval_gate
                cancel_events = [e for e in task_events_sorted if e.event_type == "BLOCKER_CANCELLED"]
                self.assertEqual(len(cancel_events), 1)
                for ce in cancel_events:
                    guard_names = {gr.guard for gr in ce.guard_results}
                    self.assertNotIn("approval_gate", guard_names)

                # ═════════════════════════════════════════════════════════
                # Worker kind variants — each attempt uses distinct kind
                # ═════════════════════════════════════════════════════════
